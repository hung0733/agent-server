...
class LLMLevelEndpointDAO:
    ...
    async def get_by_id(
        ...
        async def _query(s: AsyncSession) -> Optional[LLMLevelEndpointEntity]:
            result = await s.execute(
                select(LLMLevelEndpointEntity).where(
                    LLMLevelEndpointEntity.id == level_endpoint_id
                )
            )
    ...
    async def get_by_endpoint_id(
        ...
        async def _query(s: AsyncSession) -> Optional[LLMLevelEndpointEntity]:
            result = await s.execute(
                select(LLMLevelEndpointEntity).where(
                    LLMLevelEndpointEntity.endpoint_id == endpoint_id
                )
            )
            return result.scalar_one_or_none()
    ...
    async def delete(
        ...
        async def _delete(s: AsyncSession) -> bool:
            stmt = delete(LLMLevelEndpointEntity).where(
                LLMLevelEndpointEntity.id == level_endpoint_id
            )
    ...
    async def exists(
        ...
        async def _query(s: AsyncSession) -> bool:
            result = await s.execute(
                select(LLMLevelEndpointEntity.id).where(
                    LLMLevelEndpointEntity.id == level_endpoint_id
                )
            )
    ...
    async def get_by_agent_instance_id(
        ...
        async def _query(s: AsyncSession) -> List[LLMEndpointWithLevel]:
            ...
            all_rows = await s.execute(
                ...
                .join(
                    AgentInstanceEntity,
                    AgentInstanceEntity.endpoint_group_id
                    == LLMLevelEndpointEntity.group_id,
                )
                .where(
                    or_(
            ...
            if total_count > 0:
                logger.info(
                    _(
                        "代理 %s: 找到 %d 個端點，過濾掉 %d 個（API key 為空），保留 %d 個"
                    ),
                    agent_instance_id,
                    total_count,
                    filtered_count,
