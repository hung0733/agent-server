...
"""
Pydantic DTOs for dead letter queue.

This module provides Pydantic v2 DTOs for validation and serialization
of dead letter queue data. Follows the Base/Create/Update/Complete pattern.

Import path: db.dto.dead_letter_queue_dto
"""
...
class DeadLetterQueueCreate(DeadLetterQueueBase):
    """DTO for creating a new DLQ entry.
...
class DeadLetterQueueUpdate(BaseModel):
    """DTO for updating an existing DLQ entry.
    ...
    id: UUID = Field(
        ...,
        description="ID of the DLQ entry to update",
    )
    """ID of DLQ entry to update (required)."""
...
class DeadLetterQueue(DeadLetterQueueBase):
    """Complete dead letter queue DTO with all database fields.
