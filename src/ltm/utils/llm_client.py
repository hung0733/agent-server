...
class LLMClient:
    ...
    def chat_completion(
        ...
        for level in range(2, 0, -1):
            models = self.model_set.level[level]  # type: ignore
            for model_dto in models:
                try:
                    ...
                    is_qwen_api = (
                        model_dto.base_url
                        and "dashscope.aliyuncs.com" in model_dto.base_url
                    ) or (
                        model_dto.model_name and "qwen" in model_dto.model_name.lower()
                    )
                    ...
                    client = OpenAI(
                        base_url=model_dto.base_url, api_key=model_dto.api_key_encrypted
                    )
        ...
        raise RuntimeError(_("Failed to get response from all available models"))

    def _handle_streaming_response(self, client, **kwargs) -> str:
        """
