...
class SessionDAO:
    ...
    async def exists_by_session_id(self, session: AsyncSession, session_id: str) -> bool:
        """檢查 session_id 是否已存在"""
    ...
    async def get_by_id(self, session: AsyncSession, id: int) -> Optional[SessionModel]:
        """根據 DB ID 獲取 Session"""
        result = await session.execute(select(SessionModel).where(SessionModel.id == id))
        return result.scalar_one_or_none()
    
    async def get_by_session_id(self, session: AsyncSession, session_id: str) -> Optional[SessionModel]:
    ...
    async def get_default_session(self, session: AsyncSession, agent_db_id: int) -> Optional[SessionModel]:
    ...
    async def list_by_agent(self, session: AsyncSession, agent_db_id: int) -> List[SessionModel]:
    ...
    async def update(self, session: AsyncSession, id: int, name: Optional[str] = None) -> SessionModel:
        """更新 Session (根據 DB ID)"""
